...
const Conversation = () => {
  ...
  return (
    <section
      className=" max-h-150 lg:max-h-120 px-5 pt-2 text-base 
    selection:bg-accent selection:text-bg-primary tracking-wider text-justify overflow-y-scroll flex flex-col  gap-2"
    >
      {currentChat?.content.map(({ role, message }, index) =>
        role === "User" ? (
          <UserMessage key={index.toString()} msg={message} />
        ) : (
          <section key={index}>{message}</section>
        )
      )}
    </section>
  );
